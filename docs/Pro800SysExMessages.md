...
|0x11 | [00-1F] | Sets the current preset's **Pitch Bend Range** in semitones (0–31), in the **edit buffer**: stored as `param × 2048` in program field 196 (format 111) once the preset is saved, invisible to a `0x77` read before that. That is the semitone count in the field's top five bits with the rest zero; other writers leave other low bits, and `value >> 11` reads them all (see field 196). Status OK in that range, Status Error otherwise
...
|0x71 | [00-17] | "push" buttons (Status OK in that range, Status Error otherwise — all 104 higher codes measured):<br>  - 00-09 = numpad<br> - 0A = Arp Up-Dn<br> - 0B = Arp Assign<br> - 0C = preset — **also recalls** the preset the settings block points at, like `0x32 00`, but as a mode button it may change the panel's mode<br> - 0D = rec<br> - 0E = perf<br> - 0F = settings<br> - 10 = seq 1<br>- 11 = seq 2<br>- 12 = tune<br>- 13 = sync clock<br> - 14 = sync source<br> - 15 = accepted, does nothing (no button has this code)<br>- 16 / 17 = the value wheel: steps the **preset selection** in preset mode, steps a setting's value while that setting is in its change state, does nothing otherwise<br>Note `0x71` is also the *reply* type of `0x70`.<br>**The synth's own save is a button sequence, so it can be driven from here**: `0D` (Rec), the slot's tens digit, its ones digit, `0C` (Preset) stores the edit buffer into that slot **of the current bank**. Four caveats measured on hardware: the sequence carries no bank (set `Current Bank` first and verify afterwards that the same slot in the other banks is unchanged), it **blanks the preset name**, it stores the current knob positions along with everything else, and **Perf pressed twice is manual mode** — the edit buffer becomes the physical panel, and a save after that stores every knob and switch position rather than the preset (RE session 19 lost a trial to it). The plugin does not use it
...
* **The front panel is a second writer to the settings block**: closing a settings menu on the synth writes that menu's displayed value back over whatever the host wrote. The **Performance menus do not**: a Pitch Bend Range set by CC survived the operator opening Performance menu 5, reading it and leaving (one menu, one trial, RE session 19) — an edit sent as CC is not undone by the player looking at it.
...
|195 | 1 | Glide Mode
|196 | 2 | Pitchbend Range<br> --> a 16-bit value whose **top five bits are the semitone count** (0-31): `semitones = value >> 11`, which is what the synth's own menu displays and what `ProgramMessage::pitchBendRangeSemitones()` returns. The low eleven bits depend on who wrote the field and carry no meaning: the `0x11` message leaves them zero (`semitones × 2048`, 24 = 49152), and so does the synth when it converts an older record (12 = 24576, see "How the synth converts an older record"); a received CC 42 stores the CC byte replicated into 16 bits, `cc × 516 + 3` (CC 96 → 49539, CC 99 → 51087, CC 0 → 0); the front panel's value wheel leaves any value inside the semitone's 2048-wide band (24 was read back as 49531, 50563 and 50715 in three saves). Measured on every writer in RE session 19 (RE §13). An earlier note here, "24 semitones = 50737", was one such wheel position, not a scale — do not decode the field as `value / 65535 × 31`, which shows 24576 as 11 and 49531 as 23
